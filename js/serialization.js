    ...
    // Classification history to track all responses
    ...
    const filename = `biomarker_classification_${state.biomarkerName || 'unknown'}_${timestamp.replace(/[:.]/g, '-')}.json`;

    const exportData = {
    metadata: {
    exportDate: timestamp,
    applicationVersion: "1.0",
    biomarkerName: state.biomarkerName
},
    classification: {
    biomarkerName: state.biomarkerName,
    finalResult: classificationHistory.finalResult,
    startTime: classificationHistory.startTime,
    endTime: classificationHistory.endTime,
    duration: classificationHistory.endTime ?
    (new Date(classificationHistory.endTime) - new Date(classificationHistory.startTime)) / 1000 : null
},
    inputScores: {
    consistencyScore: state.consistencyScore,
    reproducibilityScore: state.reproducibilityScore,
    specificityScore: state.specificityScore,
    improvementGeneralHealth: state.improvementGeneralHealth,
    riskReductionChronicDisease: state.riskReductionChronicDisease,
    supportLivingWithChronicDisease: state.supportLivingWithChronicDisease
},
    thresholds: {
    tc: state.tc,
    tr: state.tr,
    ...
    settings: {
    backendEnablerActive: state.backendEnablerActive
},
    scientificEvidence: {
    validPublicationCount: state.validPublicationCount,
    validStatementCount: state.validStatementCount,
    totalScientificEvidence: state.validPublicationCount + state.validStatementCount
},
    classificationHistory: {
    responses: classificationHistory.responses,
    pathTaken: classificationHistory.pathTaken,
    stepsCompleted: classificationHistory.pathTaken.length
},
    currentState: {
    currentStepId: currentStepId,
    isComplete: isComplete,
    allStateVariables: {...state}
}
};
    ...
    // Track user response
    function trackResponse(stepId, question, response, responseType = 'user') {
    classificationHistory.responses.push({
        stepId: stepId,
        question: question,
        response: response,
    ...
    classificationHistory.pathTaken.push(stepId);
}
...
// Backend enabler toggle
    ...
    classificationHistory.responses = [];
    classificationHistory.pathTaken = [];
    classificationHistory.startTime = new Date().toISOString();
    classificationHistory.endTime = null;
    classificationHistory.finalResult = null;
    ...
    // Disable save button until completion
    ...
    case 'function':
    step.execute();
    trackResponse(step.id, step.question || 'Function executed', 'Function executed', 'system');
    if (step.next !== null) {
    ...
    const decisionResult = result ? 'Yes (condition met)' : 'No (condition not met)';
    trackResponse(step.id, 'Automated decision', decisionResult, 'system');
    ...
    isComplete = true;
    classificationHistory.endTime = new Date().toISOString();

    // Set result message based on step
    switch (currentStepId) {
    case 5:
    resultMessage = "Result: Not a good biomarker";
    classificationHistory.finalResult = "Not a good biomarker";
    break;
    case 7:
    resultMessage = "Result: Clinical use case detected - out of scope";
    classificationHistory.finalResult = "Clinical use case - out of scope";
    break;
    case 28:
    resultMessage = "Result: Unproven significance";
    classificationHistory.finalResult = "Unproven significance";
    break;
    case 29:
    resultMessage = "Result: High scientific significance";
    classificationHistory.finalResult = "High scientific significance";
    break;
    default:
    resultMessage = "Flow completed at step " + currentStepId;
    classificationHistory.finalResult = "Completed at step " + currentStepId;
}
    ...
    trackResponse(step.id, step.question, 'Yes');
    if (step.yes !== null) {
    ...
    trackResponse(step.id, step.question, 'No');
    if (step.no !== null) {
    ...
    trackResponse(step.id, step.question, 'Continue');
    if (step.next !== null) {
    currentStepId = step.next;
    renderStepIndicator();
    renderCurrentStep();
    ...
    step.update(inputValue);
    trackResponse(step.id, step.question, inputValue);
    ...
    optionsHTML += `<button class="option-button secondary" data-next="${option.next}" data-text="${option.text}">${option.text}</button>`;
});
    ...
    trackResponse(step.id, step.question, selectedText);
